...
class PaymobConfig:
    ...
    CARD_IFRAME_ID = 859704  # iFrame for card payments
    
    # IMPORTANT: Mobile wallets DON'T use iFrames!
    # They use direct redirect URL instead
    # No WALLET_IFRAME_ID needed
    ...
    IFRAME_ID = CARD_IFRAME_ID  # Use card iframe as default
    IFRAME_URL = f"https://accept.paymob.com/api/acceptance/iframes/{CARD_IFRAME_ID}"
    
    # Currency
    CURRENCY = "EGP"
    
    # Test Mode - When True, uses demo mode without calling real Paymob API
    TEST_MODE = False
    DEMO_MODE = False  # Now using real Paymob API
    ...
    def get_mobile_wallet_url(payment_token):
        """Generate redirect URL for mobile wallet payments - mobile wallets use redirect, not iFrame"""
